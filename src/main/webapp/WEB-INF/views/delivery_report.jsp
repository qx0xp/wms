...
<body>
	...
	<div class="content">
    	<h2 class="title"><div class="triangle"></div>출고현황</h2>
        <form id="actionForm" action="/ovolo/delivery_report" method="get">
        ...
        <div class="sc_lookup">
		    <table>
		        <tr>
			        <th>출고유형</th>
		            <td>
				        <select id="delivery_category" name="delivery_category" class="box setValue">
				            <option value='' selected>--선택--</option>
				            <option value="A">품목별</option>
				            <option value="B">출하지별</option>
				        </select>
				    </td>
		            <th>조회기준 날짜</th>
		    ...
		    <input class="category" type="hidden" value="${delivery_category}">
        </div>
		</form>
		...
		<c:forEach items="${delivery_report}" var="report">
			<tr class="raw1">
				<td>${report.raw_id}</td>
			</tr>
			<tr class="raw2">
			...
			<tr class="raw3">
				<td>${report.total_delivery_quantity}</td>
			</tr>
			<tr class="raw4">
...
$(document).ready(function(){
	...
	$("select[name='delivery_category']").val(category);
	...
	var td4 = $(".raw4").children();
   	...
   	var total_delivery_quantity = new Array();
   	var shipment_name = new Array();
    ...
    td3.each(function(i){
    	total_delivery_quantity.push(td3.eq(i).text());
    })  
    
    td4.each(function(i){
    	shipment_name.push(td4.eq(i).text());
    }) 
    
    console.log(raw_id);
    console.log(raw_name);
    console.log(total_delivery_quantity);
